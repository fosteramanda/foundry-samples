...
- **Adaptive Card digests** - In Teams chats, scheduled briefings and on-demand board summaries use `send_teams_digest` to post a compact card with activity/decision sections and live Planner counts, owners, statuses and due dates.
  - The card previews up to six open tasks, earliest due first, states how many are shown, and links to Planner for the full board. It does not modify tasks or hide test cards from the totals.
  - Owners are mentioned once each only when requested. Ordinary replies, email delivery, and the legacy `/workstreamsummary` command are unchanged. Existing routine instructions that name `SendMessageToChat` can use the card tool without recreating their schedules.
  - Delivery uses the agent's delegated Graph `ChatMessage.Send` permission and the current chat ID; no caller-supplied destination or card JSON is accepted. The tool is not offered for email or channel conversations.
  - Cards use schema 1.2 and link actions only, matching [Graph card support](https://learn.microsoft.com/graph/teams-messaging-overview). The 20 KiB card and 28 KiB message budgets are application limits. An uncertain POST is never automatically retried.
  - Dates use `MeetingDisplayTimeZone` (default `Pacific Standard Time`), shared with meeting displays. A date is marked past due only before today's date in that zone.
  - Run `dotnet test .\tests\AutopilotRouterAgent.Tests\AutopilotRouterAgent.Tests.csproj` for rendering, delivery, mention, pagination, size and failure-path coverage. The presentation/delivery separation follows [FoundryChartsAgent](https://github.com/davrous/FoundryChartsAgent), without adding its charting services or protocols.
...
- **Responds only when addressed** — In group chats and channels the agent doesn't answer every message. It always responds in 1:1 DMs and when explicitly @-mentioned. Messages that @-mention only other people and contain no second-person reference are skipped by a free deterministic pre-filter; the remaining ambiguous messages go to a lightweight LLM judge that decides whether the message is actually directed at the agent in context (e.g. a "can you update that ETA?" follow-up right after the agent spoke), and the agent stays silent otherwise. See [AddressedToAgentGate.cs](./src/autopilot_router_agent/AgentLogic/ResponsesApi/Helpers/AddressedToAgentGate.cs) for the full criteria.
...
- **Agent instructions:** [AgentInstructions.cs](./src/autopilot_router_agent/AgentLogic/AgentInstructions.cs)
- **MCP tools:** [ToolingManifest.json](./src/autopilot_router_agent/ToolingManifest.json) — [Learn more](https://learn.microsoft.com/en-us/microsoft-agent-365/tooling-servers-overview)
