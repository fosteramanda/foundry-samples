...
public class A365AgentApplication : AgentApplication
{
    ...
    private AgentMetadata ConstructAgentMetadataFromActivity(IActivity activity)
    {
        ...
        // A scheduled run's recipient carries no tenantId. The conversation usually has one
        // because the routine stores it in its conversation reference, but a routine created
        // before that field was persisted replays a payload with no tenant anywhere. Both
        ...
        // AADSTS900021 on every fire, silently. Measured on this very agent: a routine-driven
        // turn died here before reaching the model at all.
        //
