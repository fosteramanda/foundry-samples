...
$checks = @(
    ...
    @{ Name = "teams: channel reading";   Ok = ($manifest -match '"mcpServerName":\s*"mcp_TeamsServer"') -and ($instr -match 'Reading a Teams channel or chat'); Why = "measured: asked to summarise this week's channel discussion into a Word doc, the agent called workiq___ask and workiq___search_paths, failed to resolve the thread, and told Amanda it had no access and to paste the discussion in. It had no Teams tool at all. Work IQ search is not a way to read one known thread" }
