...
public class RoutineToolHandler
{
    ...
    private JsonObject? BuildActivityInput(string instruction)
    {
        ...
        // (the FOUNDRY_AGENT_DEFAULT_INSTANCE_CLIENT_ID environment variable). Measured on a
        // sibling agent, the two are different identities: the inbound activity carries the
        // instance the user is actually talking to, while the agent version carries the identity
        // Foundry created for the version itself, which typically holds no permission grants at
        // all. A routine built with the latter is accepted, fires on schedule, and then fails
        // every single run with
        //   AADSTS7002203: No matching federated identity record found for presented assertion
        // before the model ever runs. Nothing surfaces to the user; the output simply never
