...
- **Responds only when addressed** — In group chats and channels the agent doesn't answer every message. It always responds in 1:1 DMs and when explicitly @-mentioned. Messages that @-mention only other people and contain no second-person reference are skipped by a free deterministic pre-filter; the remaining ambiguous messages go to a lightweight LLM judge that decides whether the message is actually directed at the agent in context (e.g. a "can you update that ETA?" follow-up right after the agent spoke), and the agent stays silent otherwise. See [AddressedToAgentGate.cs](./src/agentic_colleague_agent/AgentLogic/ResponsesApi/Helpers/AddressedToAgentGate.cs) for the full criteria.
...
- **Agent instructions:** [AgentInstructions.cs](./src/agentic_colleague_agent/AgentLogic/AgentInstructions.cs)
- **MCP tools:** [ToolingManifest.json](./src/agentic_colleague_agent/ToolingManifest.json) — [Learn more](https://learn.microsoft.com/en-us/microsoft-agent-365/tooling-servers-overview)
...
Steps 1 and 2 run on **every** `azd provision`. The one-time setup steps — **publishing the autopilot**, creating the **blueprint SP OAuth2 grants**, and **adding you as blueprint owner** — are skipped on re-runs once they have completed (a `DIGITAL_WORKER_SETUP_DONE` marker is persisted in the azd environment). The published autopilot references the agent GUID, not a specific version, so new versions are served without re-publishing. There is no bot service to recreate, and the blueprint is created as part of agent creation rather than as an ARM resource.
...
- **User-Assigned Managed Identity (UMI)** — no longer deployed. It existed only to run the deployment script that created the Agent Blueprint.
- **No Bot Service, and no blueprint created in infra.** The blueprint is created by the platform when the **agent version** is created, and its client ID is returned on that response, so nothing before agent creation can know it. The agent endpoint is authorized with the **`BotServiceRbac`** scheme (PATCHed in `agent-creation-script.ps1`) rather than by a Bot Service resource.

  One consequence to know before a first deploy: the blueprint client ID does not exist at **image build** time. `post-provision.ps1` captures it from the agent-creation result, exports it for the blueprint-owner and OAuth2-grant scripts, and persists it with `azd env set`, so it is available to every later provision and injected into the agent version's environment variables. On a **brand new** environment the first agent version carries no `Connections__ServiceConnection__Settings__ClientId`; re-run the provision before investigating an auth failure.
...
|--------|--------------|-----------------------------|
| `preflight.ps1` | Preflight gate. Builds, then asserts the invariants a deploy depends on: assembly identity and namespace root, Dockerfile `ENTRYPOINT` matching the built assembly, every declared tool having a dispatch arm, every handler being both offered and dispatched, the prompt naming no tool the agent does not have, `ToolboxVersion` being pinned, `ToolboxName` matching `create-toolbox.ps1`, and no reference to another agent's Foundry account. Every assertion states what it defends, so a stale check is distinguishable from a real regression. **Exits nonzero and blocks the deploy on any failure.** Use `-SkipBuild` to reuse existing binaries. | None. Runs entirely locally against source and the built assembly. |
...
| `publish-digital-worker.ps1` | Publishes the agent as an **AI Teammate** via `POST {projectEndpoint}/agents/{agentName}/microsoft365/publish?api-version=2025-11-15-preview`, sending `publishAsAutopilot`, `publishScope` and `optionalPermissionScopes`. The agent then appears in the **Requests** tab in MAC. This is **not** the older `azureml.ms/agent-asset/v2.0` call with `publishAsDigitalWorker`; the two are not interchangeable. | **Foundry User** (or equivalent publish-capable role) on the Foundry project **+ Frontier preview** tenant enrollment. |
