        ...
        CancellationToken cancellationToken)
    {
        ...
        if (arguments is { ValueKind: JsonValueKind.Array } json)
        {
            ...
            foreach (var element in json.EnumerateArray())
            {
                ...
                {
                    // Malformed arguments are a recoverable model mistake — return a usable error to
                    // the model rather than throwing and failing the whole tool call.
                    return $"Error: file-based skill scripts only accept string CLI arguments but received a JSON element of kind '{element.ValueKind}'. All array elements must be JSON strings.";
                }
        ...
        {
            return $"Error: expected a JSON array of CLI arguments but received {arguments.Value.ValueKind}. File-based skill scripts expect positional arguments as a JSON array of strings.";
        }
