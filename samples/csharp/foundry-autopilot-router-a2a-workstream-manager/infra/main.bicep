...
//    The blueprint is created here, ahead of the agent, so its client id is stable and known to the
//    post-provision scripts that declare OAuth2 grants and inheritable scopes on it.
...
// 4. Deploy Azure Table Storage for agent data (allowlist + work items).
