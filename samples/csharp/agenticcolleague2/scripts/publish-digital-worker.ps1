...
# Fall back to the blueprint id env var when not passed in from agent creation.
if ([string]::IsNullOrEmpty($BlueprintClientId)) {
    $BlueprintClientId = $env:AGENT_IDENTITY_BLUEPRINT_ID
}
...
$body = @{
    ...
    # botServiceArmId             = "/subscriptions/$($env:SUBSCRIPTION_ID)/resourceGroups/$($env:RESOURCE_GROUP)/providers/Microsoft.BotService/botServices/$($env:AGENT_NAME)"
