...
internal class WorkIqA2AToolHandler
{
    ...
    private string ExtractAskAnswer(string? resultJson)
    {
        ...
        try
        {
            ...
            // CallMcpToolAsync may hand back either the tool result object or its content text.
            if (root is JsonValue) 
            {
                return root.GetValue<string>()?.Trim() ?? string.Empty;
            }
        ...
        {
            _logger.LogInformation(ex, "Could not parse the Work IQ MCP ask result payload.");
            return string.Empty;
        }
    }
