...
# Voice Live Bridge — LangGraph Python
...
`azure-ai-agentserver-invocations==1.1.0`. An explicit LangGraph `StateGraph` routes between a
Microsoft Foundry Responses model and a local UTC-time tool, then streams only assistant text from
the model node back to Voice Live.
...
Caller audio
    ...
    → this LangGraph hosted text agent
...
The graph is invocation-scoped and has no in-memory checkpointer. The Bridge runtime supplies
bounded history for the physical Voice connection. That history is not durable across reconnects
or hosted-agent replacement.
...
```bash
mkdir voice-live-bridge-langgraph-python
cd voice-live-bridge-langgraph-python
azd ai agent init -m https://github.com/microsoft-foundry/foundry-samples/blob/main/samples/python/hosted-agents/bring-your-own/voice-agent-target-agent/langgraph/azure.yaml
```
...
bash .github/scripts/validate-sample.sh \
  --language python \
  --sample-dir samples/python/hosted-agents/bring-your-own/voice-agent-target-agent/langgraph
```
...
- `langgraph_agent.py` constructs the workflow directly with `StateGraph`, `ToolNode`, and a
  conditional edge around a streaming `ChatOpenAI` Responses model.
- Responses are created with `store=false`; LangGraph tool messages stay inside the graph and
  are not emitted as caller-visible Bridge output.
