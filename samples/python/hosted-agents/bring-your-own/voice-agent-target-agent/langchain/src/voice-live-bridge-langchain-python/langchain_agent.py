...
"""LangChain model adapter for the Voice Live Bridge hosted-agent sample."""
...
class AgentGraph(Protocol):
    """Subset of a compiled LangChain agent used by this adapter."""
...
class LangChainAgentClient(StreamingModelClient):
    """Streaming LangChain agent backed by a Foundry Responses endpoint."""
    ...
    @classmethod
    def from_environment(cls) -> LangChainAgentClient:
        """Create a tool-using LangChain agent from hosted-agent settings."""
        ...
        async with aclosing(stream):
            async for message, _metadata in stream:
                ...
                if status is not None:
                    if status != "completed":
                        raise RuntimeError(f"LangChain model response ended with {status}")
                    completed = True
                ...
                if message.text:
                    yield message.text
        if not completed:
            raise RuntimeError("LangChain agent stream ended before model completion")
    ...
    async def close(self) -> None:
        ...
        if len(errors) == 1:
            raise errors[0]
        if errors:
            raise BaseExceptionGroup("Failed to close LangChain agent resources", errors)