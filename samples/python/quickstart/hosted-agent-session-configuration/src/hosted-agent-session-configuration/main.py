...
"""Session Configuration — hosted agent quickstart.

A minimal Responses hosted agent used to demonstrate the
``sessionConfiguration.idleTimeoutSeconds`` manifest property in
``azure.yaml``. The agent forwards user input to a Foundry model via the
Responses API and streams the reply back through the Responses protocol.

The idle timeout itself is a deploy-time property configured in
``azure.yaml`` — it is enforced by the hosting platform, not by this code.
