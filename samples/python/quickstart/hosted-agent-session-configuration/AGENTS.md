...
This sample specifically demonstrates the **`sessionConfiguration.idleTimeoutSeconds`** property in `azure.yaml`, which controls how long a hosted agent session stays alive after the last user interaction.

## Key files

- `azure.yaml` — azd manifest; `sessionConfiguration.idleTimeoutSeconds` lives under the agent service
- `src/hosted-agent-session-configuration/main.py` — agent logic
- `src/hosted-agent-session-configuration/Dockerfile` — container definition
