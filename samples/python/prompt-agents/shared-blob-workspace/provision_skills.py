#!/usr/bin/env python3
"""Provision this sample's toolbox, connection, and the workspace-mount skill on a Foundry project.

Run this ONCE (and again whenever you edit the skill) before creating the agents. It fully owns
the sample's tool surface — you do NOT pre-create a toolbox or a connection. In order:

  1. Publish skill : multipart POST {project}/skills/{name}/versions?api-version=v1
                     (each file part's filename is its path relative to the skill folder,
                     e.g. `scripts/mount_workspace.sh`; description auto-extracts from SKILL.md)
  ...
  3. Connection    : ARM PUT {PROJECT_RESOURCE_ID}/connections/{TOOLBOX_CONNECTION_NAME}
                     (category `RemoteTool`, `authType` `AgenticIdentityToken` — each agent's
                     own identity token authorizes the MCP call, `target` the toolbox MCP URL).
...
Your author-time identity needs the `Azure AI User` role on the project.
"""
import json
import mimetypes
import os
...
def _multipart_from_folder(folder: Path):
    ...
    the Skills API reconstructs the skill's directory layout (e.g. scripts/mount_workspace.sh).
    """
...
def attach_to_toolbox(endpoint, token, toolbox, skill_names):
    # GET is 404-tolerant: on the first run the toolbox does not exist yet, and the POST below
    # creates it. On later runs we preserve any existing tools on the new version.
...
def create_connection(project_resource_id, target):
    ...
    auth type: because the toolbox lives in the same project, each running agent's own identity
    token (its `Azure AI User` role) authorizes the MCP call — no key or SAS is stored on the
    connection.
    """
...
def main():
    ...
    print(f"Done. Both agents reference the toolbox via this connection (no env var needed):\n"
          f"  {conn_id}")
