...
| Registered Data Sample | [sample_registered_data.yaml](sample_registered_data.yaml) | Training job that references already-registered Foundry data assets via `azureai://` URIs and writes a named output asset via `asset_name` / `asset_version`. |
...
├── sample_training_job.yaml      # Custom code training job (local paths)
├── sample_registered_data.yaml   # Training job using registered Foundry data assets
├── src/
│   └── train.py                  # Training script invoked by the training samples
└── train_data/
